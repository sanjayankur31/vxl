...
next(const char *& s, std::istream ** is)
{
  ...
  if (*s)
    ++s; // in case s == rt+rt_pos
  rt[++rt_pos] = '\0';
  return rt[rt_pos - 1];
}

static bool
is_decimal(const char * s, std::istream ** is = nullptr)
{
  rt_pos = 0;
  char c = next(s, is);
  while (c == ' ' || c == '\t' || c == '\n' || c == '\r')
...
static bool
is_exponential(const char * s, std::istream ** is = nullptr)
{
  rt_pos = 0;
  char c = next(s, is);
  while (c == ' ' || c == '\t' || c == '\n' || c == '\r')
  ...
  c = next(s, is);
  if (c == '+')
    c = next(s, is); // no negative exponent!
  if (c < '0' || c > '9')
    return false;
  while (c >= '0' && c <= '9')
    c = next(s, is);
...
static bool
is_hexadecimal(const char * s, std::istream ** is = nullptr)
{
  rt_pos = 0;
  char c = next(s, is);
  while (c == ' ' || c == '\t' || c == '\n' || c == '\r')
...
static bool
is_octal(const char * s, std::istream ** is = nullptr)
{
  rt_pos = 0;
  char c = next(s, is);
  while (c == ' ' || c == '\t' || c == '\n' || c == '\r')
...
static bool
is_plus_inf(const char * s, std::istream ** is = nullptr)
{
  rt_pos = 0;
  char c = next(s, is);
  while (c == ' ' || c == '\t' || c == '\n' || c == '\r')
...
static bool
is_minus_inf(const char * s, std::istream ** is = nullptr)
{
  rt_pos = 0;
  char c = next(s, is);
  while (c == ' ' || c == '\t' || c == '\n' || c == '\r')
...
std::istream &
operator>>(std::istream & is, vnl_bignum & x)
{
  // decimal:     "^ *[-+]?[1-9][0-9]*$"
  // exponential: "^ *[-+]?[1-9][0-9]*[eE][+]?[0-9]+$"
  // hexadecimal: "^ *[-+]?0[xX][0-9a-fA-F]+$"
...
{
  delete[] this->data;
  this->count = 0; // Delete any allocated data
}

//: Copies rhs vnl_bignum to lhs vnl_bignum.

vnl_bignum &
vnl_bignum::operator=(const vnl_bignum & rhs)
{
  if (this != &rhs)
...
vnl_bignum
vnl_bignum::operator+(const vnl_bignum & b) const
{
  // Infinity arithmetic:
...
vnl_bignum::operator%=(const vnl_bignum & b)
{
  ...
  assert(!b.is_infinity() || !this->is_infinity()); // division Inf/Inf
  if (b.is_infinity())
    return *this; // remainder of x/Inf is x.
  if (this->is_infinity())
...
bool
vnl_bignum::operator==(const vnl_bignum & rhs) const
{
  if (this != &rhs)
  { // Check address
    if (this->sign != rhs.sign)
      return false; // Different sign implies !=
    if (this->count != rhs.count)
      return false;                           // Different size implies !=
...
std::ostream &
operator<<(std::ostream & os, const vnl_bignum & b)
{
...
std::string &
vnl_bignum_to_string(std::string & s, const vnl_bignum & b)
{
  s.erase();
...
vnl_bignum &
vnl_bignum_from_string(vnl_bignum & b, const std::string & s)
{
  // decimal:     "^ *[-+]?[1-9][0-9]*$"
  // Infinity:    "^ *[-+]?Inf(inity)?$"
...
void
vnl_bignum::dump(std::ostream & os) const
{
  ...
  { // output data array
    ...
    {
      os << ',';
      if (this->data[i - 1] < 0x10)
        os << '0';
      if (this->data[i - 1] < 0x100)
        os << '0';
      if (this->data[i - 1] < 0x1000)
        os << '0';
      os << this->data[i - 1];
    }
    os << std::dec;
...
vnl_bignum::dtoBigNum(const char * s)
{
  ...
  if (*s == '-')
    this->sign = -1; // If s had leading -, note it
...
void
vnl_bignum::exptoBigNum(const char * s)
{
  while (*s == ' ' || *s == '\t' || *s == '\n' || *s == '\r')
    ++s;                                      // skip whitespace
...
void
vnl_bignum::xtoBigNum(const char * s)
{
  this->resize(0);
...
void
vnl_bignum::otoBigNum(const char * s)
{
  this->resize(0);
  sign = 1; // Reset number to 0.
  while (*s == ' ' || *s == '\t' || *s == '\n' || *s == '\r')
...
void
vnl_bignum::resize(short new_count)
{
  assert(new_count >= 0);
...
decrement(vnl_bignum & bnum)
{
  ...
  if (bnum.count == 0)
    bnum.sign = +1; // Re-establish sign invariant
}
...
int
magnitude_cmp(const vnl_bignum & b1, const vnl_bignum & b2)
{
  if (b1.is_infinity())
    return b2.is_infinity() ? 0 : 1;
  if (b2.is_infinity())
    return -1;
  if (b1.count > b2.count)
    return 1; // If one has more data than
  if (b2.count > b1.count)
    return -1;          //   the other, it wins
...
void
multiply_aux(const vnl_bignum & b, Data d, vnl_bignum & prod, Counter i)
{
...
Data
multiply_subtract(vnl_bignum & u, const vnl_bignum & v, Data q_hat, Counter j)
{
...
void
divide(const vnl_bignum & b1, const vnl_bignum & b2, vnl_bignum & q, vnl_bignum & r)
{
  ...
  {                                     // otherwise abs(b1) > abs(b2), so divide
    ...
    { // Else full-blown divide
      vnl_bignum u, v;
#ifdef DEBUG
      std::cerr << "\nvnl_bignum::divide: b1 =";
      b1.dump(std::cerr);
...
#ifdef DEBUG
      std::cerr << "vnl_bignum::divide: q = ";
      q.dump(std::cerr);
      std::cerr << "vnl_bignum::divide: r = ";
      r.dump(std::cerr);
#endif
    }
...
vnl_bignum
left_shift(const vnl_bignum & b1, int l)
{
